...
%% ---------------------------------
processVOIP(undefined,PhInfo) ->
    q_w2p:start_qcall(PhInfo);
...
doVOIP({Session,[Streama]},PLType,L_OrigID,PhInfo) ->
	...
	{value, Aid, LPort} = q_w2p:start(Options1, Options2, PhInfo,PLType, {R_Addr,R_Port}),
...
getVOIP(Orig) when is_integer(Orig) ->
	{value, Status,_Stats} = q_w2p:get_call_status(Orig),
	{ok,Status}.
	
getVOIP_with_stats(Orig) when is_integer(Orig) ->
       case q_w2p:get_call_status(Orig) of
       {value, Status,Stats}->{ok,Status,Stats};
       R->{failed,R}
       end.
	
eventVOIP(Orig,{dial,N}) when is_integer(Orig),(is_list(N) orelse is_integer(N)) ->
    q_w2p:dial(Orig,parsePEv(N)).

start_record_rrp(AppId,Params)->
    q_w2p:start_record_rrp(AppId,Params).

stop_recording(AppId)->
    q_w2p:stop_record_rrp(AppId).
