...
inform_result1(#state{call_info=PhInfo,start_time=StartTime},Res) ->
    Qno = proplists:get_value(qno,PhInfo),
    RecDs = proplists:get_value(recds,PhInfo,""),
    Filename=proplists:get_value(qfile,PhInfo,""),
    my_result(Qno,StartTime,RecDs,Filename,Res,[{caller,proplists:get_value(cid,PhInfo)}]).
...
mylog(Fn,Fmt,Args)-> utility:log1(Fn,Fmt,Args).
