...
encode_flight(BeginEpoch, BeginSeqNo, CS0, Records) ->
    {NewEpoch, NextrySeqNo, PreEpochNextSeqNo, NexttryCS, Bin} = 
    lists:foldl(fun({?HANDSHAKE, Frag}, {Epoch, SeqNo, PreEpochNextSeqNo, CS1, DG}) -> 
    	               {RecBin2, CS11} = encode_handshake(Frag, CS1, Epoch, SeqNo),
    	               {Epoch, SeqNo+1, PreEpochNextSeqNo, CS11, <<DG/binary, (iolist_to_binary(RecBin2))/binary>>};
    	...
    	{BeginEpoch, BeginSeqNo, 0, CS0, <<>>}, Records),
    {NewEpoch, NextrySeqNo, PreEpochNextSeqNo, NexttryCS, Bin}.
...
decipher_dtls_record(Fragment, ConnnectionStates0) ->
    ...
    {T, _Mac, CipherS1} = ssl_cipher:decipher(BCA, HashSz, CipherS0, Fragment, {3,2}),
...
encode_tls_cipher_text(Type, Fragment, Epoch, SeqNo) ->
    ...
    [<<?BYTE(Type), ?BYTE(254), ?BYTE(255), ?UINT16(Epoch), ?UINT48(SeqNo), ?UINT16(Length)>>, Fragment].
...
cipher(Type, Fragment, CS0, Epoch, SeqNo) ->
    ...
    {Ciphered, CipherS1} = ssl_cipher:cipher(BCA, CipherS0, MacHash, Fragment, {3, 2}),
...
mac_hash(MacAlg, MacSecret, SeqNo, Type, Length, Fragment) ->
    ssl_tls1:mac_hash(MacAlg, MacSecret, SeqNo, Type, {254, 255},
		      Length, Fragment).
