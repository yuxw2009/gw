...
handle_info(AF,#st{peer=undefined}=ST)  ->
	if is_record(AF,audio_frame) ->
		self() ! AF,
		{noreply,ST#st{peer={AF#audio_frame.owner,AF#audio_frame.owner}}};
	true ->
		{noreply,ST}
	end;
...
% ----------------------------------
start() ->
	{ok,Pid} = my_server:start({local,vloop},?MODULE,[],[]),
	Pid.