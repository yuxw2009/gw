...
pcmMu2A(<<>>, ABin) ->
	ABin;
pcmMu2A(<<Mu:8,Rest/binary>>,ABin) ->
	A = 'A'(Mu),
	pcmMu2A(Rest,<<ABin/binary,A:8>>).
...
pcmA2Mu(<<>>, MuBin) ->
	MuBin;
pcmA2Mu(<<A:8,Rest/binary>>,MuBin) ->
	Mu = 'Mu'(A),
	pcmMu2A(Rest,<<MuBin/binary,Mu:8>>).
...
'A'(C) ->
	...
	<<_:C/binary,AC:8,_/binary>> = list_to_binary(Mu2A),
	AC.
...
'Mu'(C) ->
    ...
    <<_:C/binary,MuC:8,_/binary>> = list_to_binary(A2Mu),
    MuC.