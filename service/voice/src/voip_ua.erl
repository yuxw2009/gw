...
init(Owner,Options) ->
    ...
    Phone1 = session:trans_callee_phone(Phone,UUID),
%    io:format("voip_ua:options:~p~n",[Options]),
    init(Owner,caller,session:caller_addr(Cid),Phone1,UUID, Audit_info, Maxtime, Cid).
...
terminate(St=#state{max_talkT=MaxTalkT,timer_ref=InviteT,alertT=AlertT,owner=Owner})->
	timer:cancel(MaxTalkT),
	timer:cancel(InviteT),
	timer:cancel(AlertT),
	case St#state.uuid of
	{qvoice,_}-> void;
	{"ml",_}-> generate_cdr(St);
	{_,_}-> generate_cdr4shuobar(St)
	end,
	stop.
