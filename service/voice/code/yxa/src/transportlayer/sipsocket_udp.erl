...
-define(SIGNALLOG, "./log/signal.log").
%%====================================================================
%% External functions
%%====================================================================
...
recre()->
    F = fun(State=#state{fd=Handle})->
             file:close(Handle),
             file:rename(?SIGNALLOG, ?SIGNALLOG++utility:ts()++".log"),
             {ok, NewHd} = file:open(?SIGNALLOG, [append]),
             {ok, State#state{fd=NewHd}}
         end,
     gen_server:call(?MODULE, {command,F}).
...
init([]) ->
	...
	lists:foldl(fun(IP, Acc) ->
		    ...
		    end, [], lists:reverse([sipcfg:get(sip_socket_ip)])),    % lists:reverse(siphost:myip_list())),
    Spec =
	case yxa_config:get_env(enable_v6) of
	    {ok, true} ->
		[{udp6, {0,0,0,0,0,0,0,0}, Port} | IPv4Spec];
    ...
    start_listening(Spec, #state{socketlist = socketlist:empty(),fd=Fd}).
...
do_send2([{Socket, _SipSocket} | T], Host, Port, Msg) ->
    Res = gen_udp:send(Socket, Host, Port, Msg),
    case Res of
	{error, einval} ->
	    %% destination address not valid for this socket, try next
	    do_send2(T, Host, Port, Msg);
	Res ->
	    %%io:format("UDP send success: ~p ~p ~p~n",[Host, Port, Msg]),
	    Res
    end;
