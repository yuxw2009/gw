...
get_30_by_uuid(UUID)->get_30_by_uuid(UUID,0).    
get_30_by_uuid(UUID,StartId)->
    case get_by_uuid(UUID,StartId) of
    	{atomic,Trfs}-> 
    	    get_30_by_uuid(Trfs,0,[]);%[||#traffic{caller=Caller}<-Trfs];
    	_-> []
    end.
get_30_by_uuid([],_,R)-> R;
get_30_by_uuid(_,N,R) when N>30-> R;
get_30_by_uuid([Head=#traffic1{callee=Callee}|Rest],N,R)-> 
    SameCallees=[Itm||Itm=#traffic1{callee=C}<-Rest, C==Callee],
    Rest1=Rest--SameCallees,
    NewItem=[Head|SameCallees],
    get_30_by_uuid(Rest1,N+1,R++[NewItem]).
...
handle_traffic([])-> [];
handle_traffic(All=[#traffic1{id=Id,endtime=EndT,callee=Callee0,caller=Caller0}|_])->
