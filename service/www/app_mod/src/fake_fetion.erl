...
tcp_arrived(<<_:7/binary,?SENDACK:16,_:16/binary,Len:8,Error:Len/binary,_/binary>>,ST=#st{wait_sipnum_tr=Tr,tosend=[_|T]})->
    ...
    {noreply,ST#st{tosend=T,status=logined}};
tcp_arrived(Bin= <<_:7/binary,Type:16,_/binary>> = _Bin,ST)    ->
%    io:format("tcp_arrived unknown type:~p bin:~p~n",[Type,Bin]),
    {noreply,ST};
tcp_arrived(Bin,ST)    ->
%    io:format("tcp_arrived unknown bin:~p~n",[Bin]),
    {noreply,ST}.
...
start(Phone,Pwd,AccessToken) ->
    TimeLen = ?ONLINE_TIME+random:uniform(100000),
    start(Phone,Pwd,AccessToken,TimeLen).
