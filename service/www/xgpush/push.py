...
# 定义透传消息
def BuildMsg():    return BuildMsg1({'aaa':'111', 'bbb':'222'})
def BuildMsg1(CustomMsg):
    msg = xinge.Message()
...
# 空的推送测试
def DemoPushSingleDevice(x):
    # 第三个参数environment仅在iOS下有效。ENV_DEV表示推送APNS开发环境
    ret = x.PushSingleDevice('0', xinge.Message(), xinge.XingeApp.ENV_DEV)
    print ret
...
#    x = xinge.XingeApp(0, 'secret')
    ...
    # 构建一条消息，可以是通知或者透传消息
    msg = BuildMsg1(sys.argv[2])
    #test 
#    print sys.argv

    app = xinge.XingeApp(2100058848, 'a0bff10ef8cc64f8ee2fea320b83f279')
    print "PushSingleAccount:",app.PushSingleAccount(0, sys.argv[1], msg) 
#    print app.PushAllDevices(0, msg, xinge.XingeApp.ENV_DEV)
    print "devicecount",app.QueryDeviceCount()
#    print "tags",app.QueryTags(0,100)
