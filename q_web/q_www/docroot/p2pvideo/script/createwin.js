...
var commonTemplate =  [
    '<div id="window_{id}_warp" class="LworkV_commonwin window-container window-current"  window="{id}" style="width:{width}px; height:{height}px;top:{top}px;right:{right}px;z-index:{zIndex}">',
...
var Core = _cache = {};
Core.config = {
	createIndexid:10000,		//z-index初始值
...
Core.create = function(opt){
	...
	var top = winH - 400 ,  right = 15  ;
	
		//任务栏，窗口等数据
		if(options.from == 'server'){
		   	top  = (winH -options.height)/2 ;
		    right = (winW-options.width)/2;
		}
     	_cache.windowTemp = {"width":options.width,"height":options.height,"top":top,"right":right,"emptyW":$(window).width()-options.width,"emptyH":$(window).height()-options.height,"zIndex":Core.config.createIndexid,"id":options.id,"name":options.name,"photo":options.photo ,"status":options.status, "statuscss": (options.status == '在线' ? 'online' : 'offline') ,"Signature":options.Signature, type: options.type , service: options.service};		 
		_cache.resizeTemp = {"t":"left:0;top:-3px;width:100%;height:5px;z-index:1;cursor:n-resize","r":"right:-3px;top:0;width:5px;height:100%;z-index:1;cursor:e-resize","b":"left:0;top:-3px;width:100%;height:5px;z-index:1;cursor:s-resize","l":"left:-3px;top:0;width:5px;height:100%;z-index:1;cursor:w-resize","rt":"right:-3px;top:-3px;width:10px;height:10px;z-index:2;cursor:ne-resize","rb":"right:-3px;top:-3px;width:10px;height:10px;z-index:2;cursor:se-resize","lt":"left:-3px;top:-3px;width:10px;height:10px;z-index:2;cursor:nw-resize","lb":"left:-3px;top:-3px;width:10px;height:10px;z-index:2;cursor:sw-resize"};
...
Core.resize = function(update){
	...
	$(window).bind('resize',function(){
		...
		$("div.window-container").each(function(){
			...
			_t = _t <= 0 ? 0 : _t;
			$(this).animate({"right":_l+"px","top":_t + 10 +"px"},500);
		});
	})
	$(window).bind("scroll", function (event){
		var top = document.documentElement.scrollTop + document.body.scrollTop;	
        $("div.window-container").each(function(){
		  var _t = $(this).data("info").top;
		  $("div.window-container").css('top', _t+ top);
		});
	});	
	Core.bindWindowMove();
};
...
Core.bindWindowMove = function(){
	$(document).delegate(".header","mousedown",function(e){
		...
		lay.bind("mousemove",function(e){
			...
			//鼠标贴屏幕右侧5px内
			if(e.clientX >= parseInt(lay.width() - _w)-5){
				_l = (lay.width() - _w -10)+"px";
			}
			//窗口贴屏幕顶部10px内
			if(_t <= 10){
				_t = 10;
			...
			//窗口贴屏幕底部60px内
			if(_t >= (lay.height() - _h -20)){
			   _t = (lay.height()- _h -30)+"px";
			}
