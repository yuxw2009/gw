...
function webrtcClient(owner){
	...
	this.prepareCall = function(service, mediaParas, peerSDP){
		console.log("webrtcClient.prepareCall",service,mediaParas,peerSDP);
		var self = this;
	...
	this.terminateCall = function(){
		console.log("webrtcClient.terminateCall");
...
//		this.asCaller = true;
	...
	this.setRemoteSDP = function(sdp){
		console.log("webrtcClient.setRemoteSDP",sdp);
		if (this.asCaller){
	...
	this.onPeerCandidate = function(data) {
		console.log("webrtcClient.onPeerCandidate",data.label, data.candidate);
		if(this.pc) {
	...
	this.onGotLocalStream = function(){
		...
		return function(stream){
			console.log("webrtcClient.onGotLocalStream");
    ...
    this.pc_onICE = function(){
    	...
    	return function(event){
			console.log("webrtcClient.pc_onICE");
    		if (event.candidate) {
...
/*		    	var sdpConstraints = (curObj.service == 'video') ? {'mandatory': {
    ...
    this.pc_onRemoteStreamAdded = function(){
    	var curObj = this;
    	return function(e){
			console.log("webrtcClient.pc_onRemoteStreamAdded");
...
//    		if (curObj.getOriginalVersionNo(desc.sdp) == '2'){
				console.log("webrtcClient.pc_onGotLocalDescription");
...
//    		}
	...
	this.obtainLocalStream = function(){
		console.log("webrtcClient.obtainLocalStream");
		var curObj = this;
	    var mediaAttr = null;
	    var mediaErrorTip = '';
	    switch (this.service){
	...
	this.initiatePeerConnection = function(){
		...
		var sdpConstraints = (curObj.service == 'video') ? {'mandatory': {
                          'OfferToReceiveAudio':true, 
                          'OfferToReceiveVideo':true }} : null;
		console.log("webrtcClient.initiatePeerConnection",sdpConstraints);
		if (curObj.asCaller){
		    curObj.pc.createOffer(curObj.pc_onGotLocalDescription(), null, sdpConstraints);
	    }else{
    ...
    this.getOriginalVersionNo = function(sdpTxt){
		console.log("webrtcClient.getOriginalVersionNo");
