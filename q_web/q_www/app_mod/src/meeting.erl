...
handle(Arg, 'POST', ["lmeeting","login"]) ->
...
handle(Arg, 'POST', ["lmeeting","meetings"]) ->
    {ok, Json, _} = rfc4627:decode(Arg#arg.clidata),
...
handle(Arg, 'GET', ["lmeeting","meetings"]) ->
...
handle(Arg, 'POST', ["lmeeting","meetings",MID, "members"]) ->
   {ok, Json, _} = rfc4627:decode(Arg#arg.clidata),
   {ok, UUID} = rfc4627:get_field(Json, "uuid"),
...
handle(Arg, 'GET', ["lmeeting","meetings", MeetingId, "members"]) ->
    {ok,UUID} = yaws_api:queryvar(Arg, "uuid"),
...
handle(Arg, 'PUT', ["lmeeting","meetings", MID]) ->
   {ok, Json, _} = rfc4627:decode(Arg#arg.clidata),
   {ok, UUID} = rfc4627:get_field(Json, "uuid"),
...
handle(Arg, 'DELETE', ["lmeeting","meetings", MID]) ->
   {ok,UUID} = yaws_api:queryvar(Arg, "uuid"),
...
handle(Arg, 'PUT', ["lmeeting","meetings", MeetingID, "members", MemberID]) -> 
    {ok, Json, _} = rfc4627:decode(Arg#arg.clidata),
    {ok, UUID} = rfc4627:get_field(Json, "uuid"),
