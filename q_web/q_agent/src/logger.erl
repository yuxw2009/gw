...
init([Basename]) ->
    ...
    update_logger_status([{normal,true},{error,true},{debug,false},{console,false}], NewState1).
...
handle_info(check_logfile_size, State) ->
    case needs_rotating([debug, normal, error], 250 * 1024 * 1024, State) of
		[] ->
		    {noreply, State};
		L ->
		    %% Create rotation filename suffix from current time
